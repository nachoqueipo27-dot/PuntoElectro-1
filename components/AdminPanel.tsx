...
// Helper UI for File Input (RESPONSIVE)
const FileUploadUI = ({ currentImage, onUpload, label, isDarkMode }: { currentImage?: string, onUpload: (e: React.ChangeEvent<HTMLInputElement>) => void, label: string, isDarkMode: boolean }) => {
    ...
    return (
        <div>
            ...
            <div className={`flex flex-col sm:flex-row items-center gap-4 p-3 rounded-xl border border-dashed transition-all ${isDarkMode ? 'border-slate-600 bg-slate-900' : 'border-slate-300 bg-slate-50'}`}>
                {currentImage ? (
                    <div className="w-full sm:w-16 h-32 sm:h-16 rounded-lg overflow-hidden bg-white flex-shrink-0 border border-slate-200 relative group flex items-center justify-center p-1">
                        <img 
                            src={currentImage} 
                    ...
                    <div className="w-full sm:w-16 h-16 rounded-lg bg-slate-200 flex items-center justify-center text-slate-400 flex-shrink-0">
                        <Image size={24} />
                    </div>
                )}
                <div className="flex-1 w-full text-center sm:text-left">
                    <button 
                        ...
                        className={`cursor-pointer w-full sm:w-auto inline-flex items-center justify-center gap-2 px-4 py-2 rounded-lg text-sm font-bold transition ${isDarkMode ? 'bg-slate-700 text-white hover:bg-slate-600' : 'bg-white border border-slate-300 text-slate-700 hover:bg-slate-50'}`}
                    >
                        <Upload size={16} />
                        Subir Imagen
                    </button>
                    ...
                    />
                    <p className="text-xs text-slate-500 mt-2 sm:mt-1">Formatos: JPG, PNG. Max 5MB</p>
                </div>
            </div>
        </div>
    );
                ...
                {activeTab === 'config' && (
                    <div className="max-w-4xl mx-auto space-y-8">
                        ...
                        <div className={`p-6 rounded-2xl border space-y-6 ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
                            ...
                            <FileUploadUI label="Imagen de Fondo (Opcional)" currentImage={config.bestSellers.backgroundImage} onUpload={(e) => handleImageUpload(e, (base64) => handleConfigChange('bestSellers', 'backgroundImage', base64))} isDarkMode={isDarkMode} />
                        ...
                        <div className={`p-6 rounded-2xl border space-y-6 ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
                            ...
                            <div className="grid md:grid-cols-2 gap-6">
                             ...
                             <FileUploadUI label="Imagen de Fondo" currentImage={config.promoBanner.image} onUpload={(e) => handleImageUpload(e, (base64) => handleConfigChange('promoBanner', 'image', base64))} isDarkMode={isDarkMode} />
                        </div>
                    ...
                    <div className={`w-full max-w-2xl rounded-3xl shadow-2xl flex flex-col max-h-[90vh] ${isDarkMode ? 'bg-slate-800 text-white' : 'bg-white'}`}>
                        ...
                        <div className="p-6 overflow-y-auto flex-1 custom-scrollbar">
                            <form id="product-form" onSubmit={handleSaveProductReal} className="space-y-6">
                                ...
                                <FileUploadUI label="Imagen" currentImage={editingProduct.image} onUpload={(e) => handleImageUpload(e, (base64) => setEditingProduct(prev => prev ? {...prev, image: base64} : null))} isDarkMode={isDarkMode} />
                            </form>
                        </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
                    <div className={`w-full max-w-lg rounded-3xl shadow-xl ${isDarkMode ? 'bg-slate-800 text-white' : 'bg-white'}`}>
                        ...
                        <form onSubmit={handleSaveCategory} className="p-6 space-y-4">
                            ...
                            <FileUploadUI label="Imagen" currentImage={editingCategory.image} onUpload={(e) => handleImageUpload(e, (base64) => setEditingCategory(prev => prev ? {...prev, image: base64} : null))} isDarkMode={isDarkMode}/>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
                    <div className={`w-full max-w-lg rounded-3xl shadow-xl ${isDarkMode ? 'bg-slate-800 text-white' : 'bg-white'}`}>
                        ...
                        <form onSubmit={handleSaveBanner} className="p-6 space-y-4">
                            ...
                            <FileUploadUI label="Imagen de Fondo" currentImage={editingBanner.image} onUpload={(e) => handleImageUpload(e, (base64) => setEditingBanner(prev => prev ? {...prev, image: base64} : null))} isDarkMode={isDarkMode} />
                            <button className="w-full bg-blue-600 text-white font-bold py-3 rounded-xl mt-4">Guardar</button>
                        </form>
                    </div>
                </div>
